...
describe('FeedDateProvider', () => {
  ...
  function setup() {
    ...
    return { ...renderHook(() => useFeedDateContext(), { wrapper }), errorReporter };
  }

  it('콜드 스타트의 과거 route 날짜를 표시하지 않고 오늘로 정규화한다', () => {
    jest.setSystemTime(new Date(2026, 6, 15, 0, 0, 1));
    mockRouteDate = '2026-07-14';

    const { result, errorReporter } = setup();
  ...
  it('사용자가 과거 날짜를 보고 있어도 자정이 지나면 즉시 새 오늘로 돌아간다', () => {
  ...
  it('route 정규화 실패가 오늘 날짜 표시를 중단시키지 않는다', () => {
    jest.setSystemTime(new Date(2026, 6, 15, 0, 0, 1));
    mockRouteDate = '2026-07-14';
