...
describe('ListRow', () => {
  describe('기본 렌더링', () => {
    it('should render contents when provided', async () => {
      // Given: ListRow에 contents가 주어졌을 때
      const contentsText = '제목';

      // When: ListRow를 렌더링하면
      await render(<ListRow contents={<ListRow.Texts type="1RowTypeA" top={contentsText} />} />);

      // Then: contents가 화면에 표시된다
      expect(screen.getByText(contentsText)).toBeTruthy();
    });

    it('should render left, contents, and right sections', async () => {
    ...
    it('should render without left section', async () => {
  ...
  describe('ListRow.Texts', () => {
    it('should render only top text for 1RowTypeA', async () => {
      // Given: 1RowTypeA 타입으로 top만 주어졌을 때
      const topText = '제목';

      // When: ListRow.Texts를 렌더링하면
      await render(<ListRow.Texts type="1RowTypeA" top={topText} />);

      // Then: top만 화면에 표시된다
      expect(screen.getByText(topText)).toBeTruthy();
    });

    it('should render top and bottom for 2RowTypeA', async () => {
      // Given: 2RowTypeA 타입으로 top과 bottom이 주어졌을 때
      const topText = '제목';
      const bottomText = '부제목';

      // When: ListRow.Texts를 렌더링하면
      await render(<ListRow.Texts type="2RowTypeA" top={topText} bottom={bottomText} />);
    ...
    it('should render top, middle, and bottom for 3RowTypeA', async () => {
  ...
  describe('ListRow.Image', () => {
    it('should render image with default size', async () => {
      ...
      await render(<ListRow.Image source={imageSource} testID={testId} />);
