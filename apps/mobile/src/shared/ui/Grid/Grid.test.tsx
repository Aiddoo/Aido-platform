...
describe('Grid 컴포넌트', () => {
  ...
  it('기본적으로 flex-row, flex-wrap 클래스를 적용해야 한다', async () => {
    await render(
      <Grid testID="grid">
        <Text>콘텐츠</Text>
      </Grid>,
  ...
  it('rowGap prop을 style로 적용해야 한다', async () => {
    await render(
  ...
  it('columnGap prop을 style로 적용해야 한다', async () => {
    await render(
  ...
  it('rowGap과 columnGap을 동시에 적용해야 한다', async () => {
    await render(
  ...
  it('columns prop에 따라 GridItem의 width를 결정해야 한다', async () => {
    await render(
      <Grid columns={3}>
        <GridItem testID="item">
          <Text>아이템</Text>
        </GridItem>
      </Grid>,
    );
...
describe('GridItem 컴포넌트', () => {
  ...
  it('Grid(columns=3) 안에서 기본 width를 계산해야 한다', async () => {
    await render(
      <Grid columns={3}>
        <GridItem testID="item">
          <Text>아이템</Text>
  ...
  it('colSpan=2일 때 width를 올바르게 계산해야 한다', async () => {
    await render(
      <Grid columns={3}>
  ...
  it('Grid 없이 단독 사용 시 width가 100%여야 한다', async () => {
    await render(
      <GridItem testID="item">
        <Text>아이템</Text>
      </GridItem>,
