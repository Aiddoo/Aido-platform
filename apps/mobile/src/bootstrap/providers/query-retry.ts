 ...
 * - `TransientAuthError`: 토큰 갱신 중 일시 실패(401 위장 방지).
 * - `NetworkError`: 연결 거부(서버 재실행 시 대부분 이 형태).
 ...
 * - 일시 인프라 장애(네트워크·5xx·타임아웃·토큰 갱신 일시 실패): 넉넉한 상한까지 재시도해
 *   서버 복귀를 기다린다(콜드스타트/배포 재시작 구간에 재시도 화면 대신 로딩 유지).
 * - 나머지: 기본 상한까지 재시도한다.
 */
