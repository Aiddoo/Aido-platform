...
import { ParseTodoCommand } from "./parse-todo.command";
import { ParseTodoHandler } from "./parse-todo.handler";
...
describe("ParseTodoHandler — 자연어 투두 파싱 핸들러", () => {
	let handler: ParseTodoHandler;
	let aiProvider: Mocked<AiProvider>;
	let categoryReader: Mocked<UserCategoryReaderPort>;
	let usageMeter: Mocked<AiUsageMeter>;

	const command = (categoryId?: number): ParseTodoCommand =>
		new ParseTodoCommand(
			"내일 3시 팀 미팅",
			"user-1",
			"Asia/Seoul",
			categoryId,
	...
	beforeEach(async () => {
		const { unit, unitRef } =
			await TestBed.solitary(ParseTodoHandler).compile();
		handler = unit;
	...
	it("명시적 categoryId가 추론값을 덮어쓴다", async () => {
		const result = await handler.execute(command(3));
