...
describe("SuggestionAnalysisJob — AI 제안 분석 잡", () => {
	...
	describe("onModuleInit 스케줄러 등록", () => {
		...
		it("구 스케줄러 제거가 새 스케줄러 등록보다 먼저 호출되어야 한다", async () => {
			...
			// Then
			const removeOrder =
				mockQueue.removeJobScheduler.mock.invocationCallOrder[0] ?? 0;
			const upsertOrder =
				mockQueue.upsertJobScheduler.mock.invocationCallOrder[0] ?? 0;
	...
	describe("dispatchAnalysis", () => {
		it("최근 할 일이 있는 모든 사용자에 대해 큐에 잡을 등록해야 한다", async () => {
			...
			asMock(mockDatabase.user.findMany).mockResolvedValue(users);
