...
describe("UpdateTodoItemHandler — 하위 항목 수정 핸들러", () => {
	let handler: UpdateTodoItemHandler;
	let todoRepository: Mocked<TodoRepositoryPort>;
	let todoReadRepository: Mocked<TodoReadRepositoryPort>;

	beforeEach(async () => {
		const { unit, unitRef } = await TestBed.solitary(UpdateTodoItemHandler)
	...
	it("항목 제목·완료 상태를 수정하고 부모를 재조회한다", async () => {
		...
		const result = await handler.execute(
			new UpdateTodoItemCommand(1, 10, "user-123", {
				title: "수정",
				completed: true,
			}),
		);
	...
	it("항목이 부모에 없으면 ApplicationException(TODO_0822)을 던진다", async () => {
		...
		await expect(
			handler.execute(
				new UpdateTodoItemCommand(1, 999, "user-123", { completed: true }),
			),
	...
	it("존재하지 않는 할 일이면 ApplicationException(TODO_0801)을 던진다", async () => {
		...
		await expect(
			handler.execute(
				new UpdateTodoItemCommand(999, 1, "user-123", { title: "x" }),
			),
