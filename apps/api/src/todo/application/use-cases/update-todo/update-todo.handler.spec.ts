...
describe("UpdateTodoHandler — 할 일 부분 수정 핸들러", () => {
	...
	it("존재하지 않는 할 일이면 ApplicationException(TODO_0801)을 던진다", async () => {
		...
		// When & Then
		await expect(
			handler.execute(new UpdateTodoCommand(999, "user-123", { title: "x" })),
	...
	it("제목만 수정하면 패치를 영속화하고 응답을 재조회한다", async () => {
		...
		const result = await handler.execute(
			new UpdateTodoCommand(1, "user-123", { title: "새 제목" }),
		);
	...
	it("완료→미완료 전이 시 completedAt=null을 패치에 포함한다", async () => {
		...
		// When
		await handler.execute(
			new UpdateTodoCommand(1, "user-123", { completed: false }),
		);
	...
	it("카테고리 변경 시 소유권을 확인하고 캐시를 무효화한다 (활성 한도 재체크는 없음)", async () => {
		...
		// When
		await handler.execute(
			new UpdateTodoCommand(1, "user-123", { categoryId: 2 }),
		);
	...
	it("대상 카테고리 소유권 확인에 실패하면 패치를 영속화하지 않는다", async () => {
		...
		// When & Then
		await expect(
			handler.execute(new UpdateTodoCommand(1, "user-123", { categoryId: 9 })),
