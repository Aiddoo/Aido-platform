...
2026-03-08 (일) 11:00 KST  → 첫 분석 크론 실행
                             → 조건 충족 시, 이 직후부터 GET /ai/suggestions에 표시
2026-03-22 (일) 11:00 KST   → 위 제안들 만료 (14일 경과)
\`\`\`
...
| 계층 | 메커니즘 | 효과 |
|------|---------|------|
| **1. BullMQ jobId** | \`suggestion_{userId}_{year}-W{xx}\` (7일 보관) | 같은 주 동일 유저 중복 잡 방지 |
...
가장 빠른 첫 생성 시점은 **가입 후 돌아오는 첫 번째 일요일 KST 11:00**입니다.
이 시점에 크론이 실행되더라도 위 조건을 충족하지 못하면 빈 배열이 반환됩니다.

### 폴링 전략
- 주 1회만 갱신되므로, **앱 시작 시 1회 호출**이면 충분합니다.
- 일요일 11:00 KST 이후 호출하면 새 제안을 확인할 수 있습니다.
