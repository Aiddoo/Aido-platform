 ...
 * | 타입 | 생성 시점 | 분석 대상 |
 * |------|----------|----------|
 * | 주간 | 매주 월요일 08:00 KST | 직전 주 월~일 |
 * | 월간 | 매월 1일 08:00 KST | 전월 전체 |
 *
 * ### 안전성
 * - BullMQ jobId (7일 보관) + DB unique 제약 — 중복 생성 방지
...
| 타입 | 생성 시점 | 분석 대상 기간 |
|------|----------|--------------|
| **주간** | 매주 **월요일 08:00 KST** | 직전 주 월~일 (ISO week) |
| **월간** | 매월 **1일 08:00 KST** | 전월 1일~말일 |
...
| 계층 | 메커니즘 | 효과 |
|------|---------|------|
| **BullMQ jobId** | \`report_{TYPE}_{userId}_{periodId}\` (7일 보관) | 같은 기간 동일 유저 중복 잡 방지 |
| **DB unique** | \`@@unique([userId, type, year, period])\` | 같은 기간 리포트 중복 생성 방지 |
...
--- 주간 리포트 ---
2026-03-09 (월) 08:00 KST  → 주간 크론 실행
                             → 03-03(월)~03-08(일) 데이터 분석
                             → 가입 후 첫 주간 리포트 생성 + 푸시 알림!

--- 월간 리포트 ---
2026-04-01 (수) 08:00 KST  → 월간 크론 실행
                             → 3월 전체(03-01~03-31) 데이터 분석
                             → 가입 후 첫 월간 리포트 생성 + 푸시 알림!
\`\`\`
...
### 푸시 알림
리포트 생성 완료 시 푸시 알림이 즉시 발송되므로, 별도의 폴링이 필요 없습니다.
\`nextWeeklyAt\` / \`nextMonthlyAt\`를 사용해 UI에서 예정일을 표시할 수 있습니다.
