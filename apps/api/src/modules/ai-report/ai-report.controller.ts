...
| 타입 | 크론 시점 | 분석 대상 기간 |
|------|----------|--------------|
| **주간** | 매주 일요일 UTC 22:00 (KST **월요일 07:00**) | 직전 주 월~일 (ISO week) |
| **월간** | 매월 1일 UTC 22:00 (KST **2일 07:00**) | 전월 1일~말일 |
...
--- 주간 리포트 ---
2026-03-09 (월) 07:00 KST  → 주간 크론 실행 (일 22:00 UTC)
                             → 03-03(월)~03-09(일) 데이터 분석
                             → 가입 후 첫 주간 리포트 생성!

--- 월간 리포트 ---
2026-04-02 (목) 07:00 KST  → 월간 크론 실행 (4/1 22:00 UTC)
...
| 항목 | 설명 |
|------|------|
| \`stats.completionRate\` | 해당 기간 완료율 (%) |
| \`stats.prevCompletionRate\` | 직전 기간 완료율 (비교용, 첫 리포트는 null) |
| \`stats.streakDays\` | 연속 완료 일수 |
| \`categoryBreakdown\` | 카테고리별 총 개수/완료/완료율 |
| \`dayPatterns\` | 요일별 할 일 분포 (MON~SUN) |
| \`timePatterns\` | 시간대별 할 일 분포 (0~23시) |
| \`aiSummary\` | AI가 작성한 3~5문장 요약 (한국어) |
| \`aiTips\` | AI가 제안하는 1~3개 팁 (한국어) |

## 📱 클라이언트 통합 가이드

### 폴링 전략
- \`nextWeeklyAt\` / \`nextMonthlyAt\`를 기준으로 새 리포트 도착 시점을 예측할 수 있습니다.
- **월요일 07:00 KST 이후** 호출하면 새 주간 리포트를 확인할 수 있습니다.
...
첫 주간 리포트는 가입 후 돌아오는 **월요일 07:00 KST** 이후, 첫 월간 리포트는 **다음 달 2일 07:00 KST** 이후 조회 가능합니다.`,
	})
