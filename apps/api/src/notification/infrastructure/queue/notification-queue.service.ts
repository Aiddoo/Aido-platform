...
export class NotificationQueueService implements OnModuleInit {
	...
	enqueueFollowNew(payload: FollowNewJobData): void {
		this.#enqueueAsync(NotificationJobName.FOLLOW_NEW, payload).catch((error) => {
			this.#logger.error(
	...
	enqueueFollowMutual(payload: FollowMutualJobData): void {
		this.#enqueueAsync(NotificationJobName.FOLLOW_MUTUAL, payload).catch((error) => {
			this.#logger.error(
	...
	enqueueNudgeSent(payload: NudgeSentJobData): void {
		this.#enqueueAsync(NotificationJobName.NUDGE_SENT, payload).catch((error) => {
			this.#logger.error(
	...
	enqueueCheerSent(payload: CheerSentJobData): void {
		this.#enqueueAsync(NotificationJobName.CHEER_SENT, payload).catch((error) => {
			this.#logger.error(
	...
	enqueueBillingIssue(payload: BillingIssueJobData): void {
		this.#enqueueAsync(NotificationJobName.BILLING_ISSUE, payload).catch((error) => {
			this.#logger.error(
	...
	enqueueFriendCompleted(payload: FriendCompletedJobData): void {
		this.#enqueueAsync(NotificationJobName.FRIEND_COMPLETED, payload).catch((error) => {
			this.#logger.error(
	...
	enqueueMilestoneReached(payload: MilestoneReachedJobData): void {
		this.#enqueueAsync(NotificationJobName.MILESTONE_REACHED, payload).catch((error) => {
			this.#logger.error(
	...
	async #enqueueAsync<JobName extends keyof NotificationJobMap & string>(
		...
		await this.runtime.enqueue(NOTIFICATION_QUEUE, { name, data }, this.#jobOptions());
