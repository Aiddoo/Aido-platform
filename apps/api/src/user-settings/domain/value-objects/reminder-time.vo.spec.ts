...
describe("ReminderTime.assertValidRanges", () => {
	it("아침 리마인더가 12시 이상이면 PREFERENCE_1702", () => {
		expect(() =>
			ReminderTime.assertValidRanges({ morningReminderHour: 13 }),
		).toThrow(expect.objectContaining({ errorCode: "PREFERENCE_1702" }));
	});

	it("저녁 리마인더가 12시 미만이면 PREFERENCE_1702", () => {
		expect(() =>
			ReminderTime.assertValidRanges({ eveningReminderHour: 11 }),
		).toThrow(expect.objectContaining({ errorCode: "PREFERENCE_1702" }));
	});
