...
export class CacheService {
	...
	// 공유 커널이 모듈 도메인 타입에 의존하지 않도록 제네릭으로 열어둔다.
	// (구체 타입은 호출부(todo-category 모듈)가 지정)

	async getTodoCategories<T>(userId: string): Promise<T[] | undefined> {
		return this.get<T[]>(CacheKeys.todoCategories(userId));
	}
