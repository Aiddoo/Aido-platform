...
import { asJob, flushPromises } from "@test/mocks";
...
describe("BullMQReminderSchedulerAdapter — BullMQ 리마인더 스케줄러 어댑터", () => {
	...
	describe("scheduleReminder", () => {
		...
		it("같은 todoId로 재호출하면 기존 잡을 제거하고 새로 등록한다", async () => {
			...
			queue.getJob.mockResolvedValue(asJob(mockJob));
		...
		it("잡 등록 시 올바른 delay 값이 전달된다", async () => {
			...
			const firstCall = queue.add.mock.calls.find(
				(call) => call[2]?.jobId === "reminder_1_60min",
			);
	...
	describe("cancelReminder", () => {
		...
		it("잡 제거 중 에러가 발생해도 다른 잡 제거에 영향을 주지 않는다", async () => {
			...
			queue.getJob
				.mockResolvedValueOnce(asJob(failJob))
				.mockResolvedValueOnce(asJob(successJob))
