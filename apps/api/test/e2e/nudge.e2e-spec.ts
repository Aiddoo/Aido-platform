		...
		describe("POST /nudges - 콕 찌르기", () => {
			...
			it("친구가 아닌 사용자에게 콕 찌르기 시 403 에러 반환", async () => {
				...
				const todoResponse = await request(app.getHttpServer())
					...
					.send({ title: "Stranger Todo", startDate: today });
			...
			it("자기 자신에게 콕 찌르기 시 400 에러 반환", async () => {
				...
				const todoResponse = await request(app.getHttpServer())
					.post("/todos")
					.set("Authorization", `Bearer ${sender.accessToken}`)
					.send({ title: "Self Todo", startDate: today });
	...
	describe("콕 찌름 목록 조회", () => {
		...
		beforeAll(async () => {
			...
			const todoResponse = await request(app.getHttpServer())
				.post("/todos")
				.set("Authorization", `Bearer ${receiver.accessToken}`)
				.send({ title: "목록 조회 테스트 할일", startDate: today });
	...
	describe("읽음 처리", () => {
		...
		beforeAll(async () => {
			...
			const todoResponse = await request(app.getHttpServer())
				.post("/todos")
				.set("Authorization", `Bearer ${receiver.accessToken}`)
				.send({ title: "읽음 처리 테스트 할일", startDate: today });
			const todoId = todoResponse.body.data?.todo?.id;
