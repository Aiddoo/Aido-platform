...
modules/{name}/
  domain/
    entities/{name}.entity.ts        # AggregateRoot 상속, private ctor + reconstitute
    events/*.event.ts                # plain readonly-param 클래스
    value-objects/*.vo.ts            # EntityId/ValueObject 상속, static create에서 불변식 검증
  application/
    ports/*.port.ts                  # Symbol 토큰 + 인터페이스 (파일당 1포트)
    use-cases/<kebab>/               # 커맨드 1개 = 엔드포인트 1개 (SRP)
      <kebab>.command.ts / .handler.ts / .handler.spec.ts
    queries/                         # 쿼리 + queries/handlers/
    events/*.handler.ts              # @EventsHandler 부수효과 핸들러
  infrastructure/adapters/*.ts       # 포트 구현 — 레거시 Repository/서비스에 위임(쿼리 중복 금지)
```
...
- 애그리게잇: `private constructor` + `static reconstitute(props)`. 행동 메서드에서만 상태 전이 + `apply(event)`
- `static create()` 팩토리는 autoincrement id 제약으로 미사용 (엔티티 JSDoc 참조)
- VO: `static create()`에서 불변식 검증, 위반 시 `DomainException` (예: `TodoId`, `TodoSchedule`)
