...
| 모듈 | 설명 |
|------|------|
| **Auth** | JWT 인증, OAuth (Google, Apple), 회원 관리, 세션 관리 |
| **Todo** | 할 일 CRUD, 완료 처리, 날짜/시간 관리 |
...
┌─────────────────────────────────────────────────────────┐
│  응답: ParsedTodo                                       │
│  - title, startDate, endDate, scheduledTime, isAllDay   │
└─────────────────────────────────────────────────────────┘
```

**클라이언트 플로우**: AI 파싱 → 사용자 확인/수정 → POST /v1/todos로 생성

### 소셜 모듈

| 모듈 | 설명 |
|------|------|
| **Follow** | 사용자 간 팔로우/언팔로우, 팔로워/팔로잉 목록 |
| **Cheer** | 팔로잉 대상에게 응원 메시지 전송 |
| **Nudge** | 팔로잉 대상에게 찌르기 알림 전송 |

### 통계 모듈

| 모듈 | 설명 |
|------|------|
| **DailyCompletion** | 사용자별 일일 Todo 완료율 집계 |
