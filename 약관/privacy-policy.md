...
| 서비스 제공 | 할 일 관리, 캘린더(주간/월간), 친구 공유, 콕 찌르기, 주간 성취 배지, 스트릭(연속 달성) 등 핵심 기능 제공. 공개 설정된 할 일은 맞팔(상호 팔로우) 친구에게 표시되며, 콕 찌르기 메시지는 상대방에게 전달됩니다. |
| AI 기능 제공 | 할 일 자동 파싱, 주간·월간 AI 리포트 생성(할 일 통계 분석·성취 요약), 반복 패턴 분석 및 자동 제안 등 AI 기반 기능 |
| 구독 및 결제 관리 | 유료 구독 처리, 결제 상태 관리, 영수증 검증 |
| 알림 발송 | 할 일 리마인더(시작 1시간 전·10분 전), 아침·저녁 리마인더, 스트릭 유지 알림, 소셜 알림(친구 요청, 콕 찌르기, 소셜 다이제스트 등), 콕 찌르기 유도 알림, Win-back 알림, 서비스 공지 |
...
| 수탁자 | 위탁 업무 | 처리 항목 |
|--------|----------|----------|
| **RevenueCat, Inc.** | 구독 결제 관리, 영수증 검증 | 구독 상태, 상품 ID, 구매 이력 |
...
| **RevenueCat, Inc.** | 미국 | 구독 상태, 구매 이력, 사용자 ID | 구독 결제 관리 | 서비스 이용 기간 + 관련 법령에 따른 보관 기간 |
...
1. 서비스는 다음의 AI 기능을 제공하기 위해 회원의 데이터를 외부 AI 서비스(Google Gemini API)로 전송합니다.
   ...
   - **주간·월간 AI 리포트**: 할 일 통계(완료율, 카테고리별 분석, 요일·시간대별 패턴 등 집계 데이터)를 전송하여 성취 요약 및 맞춤 팁 생성
   - **반복 패턴 분석**: 할 일 제목 및 시간 데이터를 전송하여 반복 패턴 감지 및 제안 생성
...
1. 본 개인정보처리방침은 **2026년 3월 8일**부터 시행합니다.
2. 이전 개인정보처리방침(2026년 3월 5일 시행)은 본 방침 시행과 동시에 효력을 상실합니다.
