...
export const createNudgeSchema = z
  .object({
    ...
    todoId: z.string().cuid('유효하지 않은 Todo ID입니다').describe('찔러줄 할 일의 ID'),
    message: z
      .string()
...
export const markNudgeReadSchema = z
  .object({
    nudgeId: z.string().cuid('유효하지 않은 콕 찌름 ID입니다').describe('확인할 콕 찌름 ID'),
  })
...
export const markNudgesReadSchema = z
  .object({
    nudgeIds: z
      .array(z.string().cuid('유효하지 않은 콕 찌름 ID입니다'))
